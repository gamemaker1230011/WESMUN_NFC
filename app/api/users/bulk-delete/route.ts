...
interface BulkDeleteBody { userIds?: string[] }
...
export async function POST(request: Request) {
  try {
    ...
    for (const id of userIds) {
      ...
      if (!role) { missing.push(id); continue }
      if (id === user.id || role === 'admin' || role === 'security' || role === 'overseer') { forbidden.push(id); continue }
      deletable.push(id)
    }
